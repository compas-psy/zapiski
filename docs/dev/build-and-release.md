...
Отдельная ветка и **ручная** сборка: на сервер и в фид обновлений не уезжает
ничего. Workflow — `.github/workflows/build-macos.yml`, раннер `macos-14`,
цель `universal-apple-darwin`. Не собралось universal — собирается `aarch64` и
печатается `::warning::`: молча суженная сборка это Intel-Мак, на котором
приложение не запускается, и никто не знает почему.

**Подписи Apple не будет.** Developer Program (99 $/год) у нас нет, значит нет
ни сертификата Developer ID, ни нотаризации, и Gatekeeper при первом запуске
скажет «не удалось проверить разработчика». Это объясняется человеку на
странице `/macos-warning` (`apps/web/public/macos-warning/`), а не
замалчивается; команды с `xattr` там нет намеренно — она снимает карантин
вслепую с любого файла.

Подпись **ad-hoc** (`APPLE_SIGNING_IDENTITY: '-'`) сертификата не заменяет и
Gatekeeper не успокаивает. Она нужна для другого: на Apple Silicon бинарь без
любой подписи не запускается вовсе. Поэтому она проверяется отдельным шагом
(`codesign --verify --deep --strict`), а не подразумевается.

Что различается между Windows и macOS в одном коде — без флагов сборки,
решение учредителя от 19.08.2026:

| Что | Windows | macOS |
| --- | --- | --- |
| Рамки окна | снимаются в коде при старте | `decorations: true` + `titleBarStyle: Overlay` |
| Кнопки окна | рисуем сами (`TitleBar`) | системный «светофор», слева поле 78 px |
| Открытие `.md` | путь в аргументах процесса | Apple Event → `RunEvent::Opened` |
| Глобальный хоткей | `Ctrl+Alt+N` | `Cmd+Alt+N` |
| Иконка в трее | цветная, из иконки приложения | монохромная `menubar-Template.png` |
| Автозапуск | ключ реестра | login item (`MacosLauncher::LaunchAgent`) |
| Биометрия | Windows Hello | нет; деградация до пароля |

`decorations: false` на macOS уносит вместе с полосой заголовка и три
системные кнопки — окно нельзя закрыть мышью. Поэтому в конфиге стоит
macOS-случай, а Windows снимает рамки в коде. Обе половины связки стережёт
`packages/app/test/titlebar.test.tsx`.

**Чего ещё нет:** Keychain и Touch ID. Провайдер биометрии на macOS
отсутствует (команда Windows Hello там не существует, `invoke` отклоняется,
провайдер `null`), поэтому тумблер скрыт и шифрование работает по паролю.
Это деградация, а не отказ, — но и не реализация.
...
В отличие от MSIX для Windows, отдельного шага упаковки не требуется: RuStore
принимает тот же подписанный релизный APK, что и прямая раздача с сайта —
`simpas-zapiski-<версия>.apk` из `/updates/latest/zapiski.apk` (правило имён —
[contributing.md](contributing.md#имена-сборок-правило-симпас-одно-на-три-продукта)).
Загружается он в
RuStore Консоль руками, отдельного workflow под это в репозитории нет.

Что важно перед первой загрузкой:

- **Политика конфиденциальности — черновик.** `docs/legal/README.md` помечает
  её прямо: «ЧЕРНОВИК. Не публиковать без юридической проверки», а в тексте по
  адресу `/legal/privacy` — плейсхолдеры вида `{{ФИО ОПЕРАТОРА}}` вместо
  реквизитов. RuStore требует рабочую ссылку на политику при публикации — это
  реквизиты юрлица/ИП, их не восстановить из кода, только руками до похода в
  консоль.
- **Зависимости от Google Play Services нет.** Единственный след —
  `com.google.android.gms.permission.AD_ID` и
  `com.google.android.finsky.permission.BIND_GET_INSTALL_REFERRER_SERVICE` в
  `apps/mobile/android-permissions.txt`, оба приходят транзитивно из
  AppMetrica и не блокируют запуск на устройстве без GMS — сама аналитика на
  это рассчитана и деградирует.
- **Свой апдейтер и апдейтер магазина — два независимых канала.**
  `apps/mobile/src/platform/updater.ts` ходит на собственный фид
  (`/api/v1/updates/android/{версия}`) и открывает ссылку на APK в браузере —
  это не завязано на источник установки и продолжит работать для тех, кто
  поставил ЗАПИСКИ с сайта. Для тех, кто поставил из RuStore, обновление
  предложит и сам магазин; конфликта это не создаёт (оба ведут на установку
  той же версии), но при желании держать канал единым свой апдейтер можно
  условно отключать по источнику установки
  (`PackageManager.getInstallSourceInfo`) — на момент написания это не
  сделано.
- **Иконка готова**, скриншотов и графики для карточки в каталоге в
  репозитории нет — `apps/mobile/src-tauri/icons/icon.png` (512×512) подходит
  под витрину, остальное собирается вручную под требования консоли на момент
  публикации (они меняются чаще, чем стоит фиксировать числом в этом файле).
