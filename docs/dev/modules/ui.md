...
**Состояние: закрыто 2026-08-09. Все 90 пар проходят AA**, реестр
`KNOWN_DEVIATIONS` пуст.

Три значения не проходили порог, и ни одно из них не подгонялось молча: они
были вынесены дизайнеру отдельным брифом
([`../../design/CONTRAST-BRIEF.md`](../../design/CONTRAST-BRIEF.md)) с числами и
вариантами, решение принято им:

| Токен | Было | Стало | Контраст на `--bg` |
| --- | --- | --- | --- |
| `--text-secondary` (светлые темы) | `#8A8375` | `#726C60` | 3.60 → **4.99** |
| `--accent` (`gold`, светлые) | `#B08430` | `#8F6B26` | 3.25 → **4.68** |
| `--accent-on-soft` (`gold`, светлые) | `#8E6A24` | `#886522` | 4.19 → **4.52** (на `--accent-soft`) |

Реестр оставлен пустым намеренно: тест падает и в обратную сторону — новое
отклонение он назовёт по имени, а не пропустит молча. Полная матрица чисел —
`packages/ui/CONTRAST.md`, перепечатать:
`ZAPISKI_CONTRAST_TABLE=1 pnpm --filter @zapiski/ui test`.

> Отклонение от `DESIGN_TOKENS.md` в трёх значениях — сознательное и
> согласованное: ТЗ требовало одновременно и точных hex, и порога 4.5:1, а
> выполнить оба требования было нельзя. Бриф — обоснование, почему уступила
> буква спецификации, а не порог доступности.

⚠️ **Закрыт только список DoD.** Ответ дизайнера
(`../spec/CONTRAST-DECISION.md`) содержит ещё три правки — они касаются пар
**вне** пяти проверяемых: текста бейджей на функциональных подложках (нужны
роли `--success-text` / `--warning-text` / `--danger-text`) и сдвига шкалы
вторичного текста на ступень (`--text-tertiary` при 2.09:1 — край
читаемости). **В `tokens.css` их ещё нет**, и тест их не ловит: он проверяет
ровно пять пар из DoD. Формально мы зелёные, фактически ценник на paywall
по-прежнему низкоконтрастный.
...
> Markdown» (`VOICE.md`, P1). Сама функция не реализована и в текущей сборке
> недоступна: это готовые кирпичи без сценария. Экранов `3a`–`3e` из
> `SCREENS.md` в `packages/app` нет.
