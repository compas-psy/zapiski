...
> **Состояние 0.1.0.** Собраны все слои продукта: ядро (`packages/core`),
> редактор (`packages/editor`), дизайн-система (`packages/ui`), экраны
> (`packages/app`), три платформенные оболочки (`apps/web`, `apps/desktop`,
> `apps/mobile`) и облачный бэкенд (`server`). Приложение запускается:
> `pnpm dev` поднимает веб-версию. 695 автотестов, покрытие ядра 94 %.
>
> **Чего это не значит.** Ни один клиентский артефакт не собран и не запущен:
> установщика Windows и APK не выпускали, релизных тегов не ставили,
> развёртывания на сервер не было. Облако Записок и Яндекс.Диск в интерфейсе не
> подключаются. Вся документация ниже помечает значком ⏳ то, что описано в
> спецификации, но в текущей сборке недоступно. Полная сверка —
...
| [dev/modules/ui.md](dev/modules/ui.md) | Токен-слой (3 темы × 3 акцента), правило «ни одного hex вне токенов», компоненты и их пропсы |
...
| [dev/modules/app.md](dev/modules/app.md) | Экраны, `AppHost`, состояние, раскладки, матрица состояний, палитра команд |
| [dev/modules/platforms.md](dev/modules/platforms.md) | Три оболочки: что реализовано на каждой, какие порты `null` и почему |
...
| [dev/build-and-release.md](dev/build-and-release.md) | Сборка веба/Windows/Android, пять workflow'ов, автообновление, секреты |
| [dev/contributing.md](dev/contributing.md) | Язык комментариев, приоритет ТЗ, правило ADR, границы пакетов, инварианты |

Безопасность — отдельный каталог, владелец инфобез. Читается и цитируется,
**не редактируется** командой документации:

| Документ | О чём |
| --- | --- |
| [dev/security/THREAT-MODEL.md](dev/security/THREAT-MODEL.md) | Модель угроз: активы, границы доверия, нарушители, принятый остаточный риск |
| [dev/security/AUDIT.md](dev/security/AUDIT.md) | Журнал аудита: находки `SEC-***` со статусами и что проверить не удалось |
...
| [user/sync.md](user/sync.md) | Способы хранения, что происходит при конфликте |
| [user/encryption.md](user/encryption.md) | Шифрование, пароль, отпечаток, автозамок, что видно на диске |
...
| [design/CONTRAST-BRIEF.md](design/CONTRAST-BRIEF.md) | Бриф дизайнеру по контрасту токенов. **Закрыт 2026-08-09**: три значения изменены по решению дизайнера, все 90 пар из DoD проходят 4.5:1 | Дизайн |
| [dev/security/](dev/security/) | Модель угроз и журнал аудита | Инфобез |
