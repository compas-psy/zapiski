...
const schema = z.object({
  ...
  /**
   * Ключ шифрования тома: 64 шестнадцатеричных знака (32 байта, AES-256-GCM).
   *
   * Обязателен и без значения по умолчанию — намеренно. Ключ по умолчанию был бы
   * одинаков у всех, кто выложит этот код, то есть не ключом, а украшением; а
   * необязательный ключ означал бы «том иногда открытый», и узнать, что выпал
   * именно этот раз, можно было бы только по факту утечки. Поэтому сервер без
   * него не стартует.
   *
   * ВНИМАНИЕ при эксплуатации: смена этого ключа делает НЕЧИТАЕМЫМИ все уже
   * лежащие в томе файлы. Это не «разлогинит всех», как AUTH_SECRET, — это
   * потеря содержимого. Скрипт выкладки генерирует ключ ровно один раз, при
   * отсутствии, и больше к нему не прикасается.
   */
  BLOB_ENCRYPTION_KEY: z
    .string()
