...
export async function registerLegalRoutes(app: FastifyInstance): Promise<void> {
  const document = async (slug: LegalSlug, version: string): Promise<string> => {
    ...
    */
    ...
    /*
      Баннер живёт, пока в тексте остаётся хоть один `{{…}}`: часть реквизитов
      Оператора уже заполнена владельцем (ФИО, ОГРНИП, ИНН, адрес — пакет от
      29 августа 2026), но контактные email ещё нет. §18/§21 запрещают их
      выдумывать, поэтому страница обязана честно называть себя рабочей
      редакцией ровно до тех пор, пока плейсхолдер виден в тексте, — не дольше
      и не меньше, а не по одной хардкодной фразе, которая пережила бы и
      полностью заполненный пакет.
    */
    const isDraft = text.includes('{{');
    const draft = isDraft
      ? '<p class="draft">Рабочая редакция пакета. Реквизиты Оператора, отмеченные в тексте как ' +
        '<code>{{…}}</code>, будут заполнены владельцем перед финальной публикацией.</p>'
      : '';
