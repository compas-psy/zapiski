/**
 * SEC-001 — приёмка zero-knowledge для аккаунта, который шифрует.
 *
 * ── ЧТО ЭТОТ ФАЙЛ НЕ УТВЕРЖДАЕТ (читать первым) ─────────────────────────────
 *
 * Он НЕ описывает путь MVP. Решением владельца сквозное шифрование убрано из
 * MVP и вернётся позже, когда ключ будет приходить из внешней ключницы. На
 * обычном сегодняшнем пути клиент шлёт содержимое как есть, и сервер
 * прочитать его МОЖЕТ — гарантия там другая и уже: том зашифрован ключом
 * сервера, см. `blob.at-rest.test.ts`.
 *
 * Оставлен файл потому, что проверяемое им свойство осталось верным для
 * аккаунта, который шифрование включил, и потому, что он же будет приёмкой
 * возврата шифрования. Но выводить из его зелёного цвета «ЗАПИСКИ —
 * zero-knowledge» нельзя: он проверяет сервер против шифрующего клиента,
 * которого на пути MVP нет.
 *
 * ── Что проверяется ─────────────────────────────────────────────────────────
 *
 * Это не модульный тест крипто-функций (те лежат в
 * `packages/core/test/sync-keys.test.ts`), а сквозная проверка на НАСТОЯЩЕМ
 * сервере, с настоящим Postgres и настоящим томом блобов на диске:
 *
 *   получив от клиента шифротекст, сервер не получает содержимое обычной
 *   приватной заметки, CRDT-апдейт или версию в открытом виде и не обладает
 *   ключом, которым способен их расшифровать.
 *
