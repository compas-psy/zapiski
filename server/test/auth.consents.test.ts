...
describe.skipIf(noDatabase())('документы для согласий', () => {
  ...
  for (const [path, needle] of [
    ...
    it(`${path} открывается страницей, а не JSON`, async () => {
      ...
      /* Реквизиты Оператора (ФИО, ОГРНИП, ИНН, адрес, email) заполнены
         владельцем — пометка «Рабочая редакция» гаснет сама, когда в тексте
         не остаётся ни одного плейсхолдера `{{…}}` (behaviour и весь набор
         состояний баннера проверены в legal.documents.test.ts, здесь дублировать
         незачем). Эта проверка — про другое: что ссылка с экрана входа
         действительно открывает страницу с текстом, а не тишину или JSON. */
    });
  }
