...
describe.skipIf(noDatabase())('документы для согласий', () => {
  ...
  for (const [path, needle] of [
    ...
    it(`${path} открывается страницей, а не JSON`, async () => {
      ...
      /* Пока текст не выверен юристом, человек видит это прямо на странице:
         документ, который выглядит утверждённым, не будучи им, — обман. */
      expect(response.body).toContain('Черновик');
    });
  }
  ...
  it('политика перечисляет ровно то, что собирается', async () => {
    ...
    const response = await harness.app.inject({ method: 'GET', url: '/privacy' });
    for (const item of ['почт', 'Яндекс ID', 'идентификатор устройства']) {
      expect(response.body, `в политике не упомянуто: ${item}`).toContain(item);
    }
    /* И то, чего нет: телефона мы не спрашиваем нигде (ТЗ §5.5). */
    expect(response.body).toContain('Телефон не запрашивается');
  });
