...
describe('публичные страницы API проведены через nginx', () => {
  ...
  it('адрес возврата после входа доводится до оболочки приложения', async () => {
    /* ── Дефект ───────────────────────────────────────────────────────────
       Вход через СИМПАС отработал весь круг: код обменян, подпись проверена,
       токен выдан — и браузер увели на
       `https://zapiski.cmpas.ru/auth/callback#access_token=…`, где его встретил
       голый `404 Not Found nginx`.

       Серверная половина при этом была исправна целиком, и ни одна проверка
       ничего не заметила: браузерные прогоны ходят по собранной странице, а не
       по боевому nginx, а этот файл до сих пор стерёг только страницы САМОГО
       API (`/terms`, `/p/:slug`) — адрес, на который API УВОДИТ, не стерёг
       никто.

       Адрес берётся из docker-compose.yml, а не переписывается сюда руками:
       именно оттуда он попадает в контейнер, и разойтись эти два файла могут
       молча. Промо-фолбэк (`location /`) провалить проверку обязан: он ответит
       200 и покажет знакомую страницу, но токен лежит во ФРАГМЕНТЕ, и прочесть
       его умеет только оболочка приложения. */
    const compose = await readFile(
      path.resolve(fileURLToPath(new URL('../../deploy/docker-compose.yml', import.meta.url))),
      'utf8',
    );
    const configured = /^\s*AUTH_SUCCESS_REDIRECT:\s*(\S+)\s*$/m.exec(compose);
    expect(configured, 'в docker-compose.yml нет AUTH_SUCCESS_REDIRECT').not.toBeNull();

    const target = new URL(configured![1]!).pathname;
    const block = blocks.find(
      (candidate) => candidate.modifier === '^~' && target.startsWith(candidate.pattern),
    );

    expect(
      block,
      `адрес возврата после входа (${target}) не перехвачен ни одним блоком ^~ — ` +
        'он попадёт в общий фолбэк на промо, а токен во фрагменте прочитать будет некому',
    ).toBeDefined();

    /* Сверяется ПОСЛЕДНИЙ аргумент try_files целиком, а не «оканчивается на
       /index.html»: `/promo/index.html` тоже оканчивается на него, и первая
       версия этой проверки промо-фолбэк пропускала — то есть не ловила ровно
       ту из двух поломок, которую громче всего обещала поймать. */
    const tryFiles = /try_files\s+([^;]+);/.exec(block!.body);
    expect(tryFiles, `в блоке ${block!.pattern} нет try_files`).not.toBeNull();
    const fallback = tryFiles![1]!.trim().split(/\s+/).at(-1);

    expect(
      fallback,
      `блок ${block!.pattern} доводит адрес возврата до «${fallback}», ` +
        'а сессию подхватывает только оболочка приложения /index.html',
    ).toBe('/index.html');
  });
