...
/**
 * Снятие ключа аккаунта.
 *
 * Понадобилось из-за решения владельца убрать сквозное шифрование из MVP и
 * вернуть его позже через внешнюю ключницу. У аккаунта, который успел пройти
 * онбординг, на сервере лежит шифротекст, а `assertEnvelopeIfEncrypted` не даст
 * записать поверх него открытый текст — и правильно сделает. Без способа снять
 * ключ такой аккаунт остался бы заперт навсегда: клиент больше не шифрует,
 * сервер больше не принимает.
 *
 * Опасность операции названа прямо: снятие лишает ОСТАЛЬНЫЕ устройства
 * возможности развернуть ключ по коду восстановления. Поэтому ответ отдаёт
 * снятый материал обратно — обрыв связи посреди перевода не имеет права
 * оставить человека без ключа и с шифротекстом.
 */
describe.skipIf(noDatabase())('снятие ключа аккаунта: DELETE /api/v1/vault/sync-key', () => {
  let harness: Harness;
  ...
  const payload = {
    wrappedSmk: Buffer.from('обёртка-для-снятия').toString('base64'),
    accountSalt: Buffer.from('соль-для-снятия').toString('base64'),
    checkBlob: Buffer.from('проверка-для-снятия').toString('base64'),
  };

  async function enrol(user: { authHeader: Record<string, string> }): Promise<void> {
    const put = await harness.app.inject({
      method: 'PUT',
      url: '/api/v1/vault/sync-key',
      headers: user.authHeader,
  ...
  it('после снятия аккаунт снова принимает открытый текст', async () => {
    const user = await createUser(harness);
    await enrol(user);

    const blocked = await harness.app.inject({
      method: 'PUT',
      url: '/api/v1/vault/blob/заметка.md',
      headers: { ...user.authHeader, 'content-type': 'application/octet-stream' },
      payload: Buffer.from('# открытый текст'),
    });
  ...
  it('снятый материал возвращается — обрыв посреди перевода не запирает человека', async () => {
    const user = await createUser(harness);
    await enrol(user);
    ...
    const body = removed.json() as { removed: boolean; wrappedSmk?: string; accountSalt?: string };
    ...
    expect(get.json()).toEqual({ enrolled: false });
  });

  it('повтор — успех, а не ошибка: перевод обязан быть идемпотентным', async () => {
    const user = await createUser(harness);
    await enrol(user);
    ...
    expect(second.statusCode).toBe(200);
    expect((second.json() as { removed: boolean }).removed).toBe(false);
  });

  it('снятие не трогает ключ соседа', async () => {
    const mine = await createUser(harness);
    const neighbour = await createUser(harness);
    await enrol(mine);
    await enrol(neighbour);

    await harness.app.inject({
      method: 'DELETE',
      url: '/api/v1/vault/sync-key',
      headers: mine.authHeader,
    });

    const get = await harness.app.inject({
      method: 'GET',
      url: '/api/v1/vault/sync-key',
      headers: neighbour.authHeader,
    });
    expect((get.json() as { enrolled: boolean }).enrolled).toBe(true);
  });

  it('без токена не снимает ничего', async () => {
    const response = await harness.app.inject({
      method: 'DELETE',
      url: '/api/v1/vault/sync-key',
    });
