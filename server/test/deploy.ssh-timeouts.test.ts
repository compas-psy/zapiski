...
/**
 * Вызов ssh/rsync ГДЕ УГОДНО в строке, а не только в её начале.
 *
 * `rsync` здесь наравне с `ssh`: файлы на сервер едут им (см.
 * `deploy.transport.test.ts`), и повиснуть он может ровно так же.
 *
 ...
 */
const SSH_CALL = /(?:^|[|;&]\s*|\s)(?:ssh|scp|rsync|ssh-keyscan)\s+[-\w~"'$]/;
