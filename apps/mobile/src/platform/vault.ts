 ...
 *
 * На Android нет «выбери любую папку» в том смысле, в каком он есть на
 * Windows: произвольный каталог общей памяти приложению недоступен (scoped
 * storage), а SAF отдаёт `content://`-дерево, поверх которого нет ни
 * `rename`, ни атомарной записи. Поэтому vault лежит в каталоге приложения
 * во внешней памяти — это настоящие файлы `.md` на настоящей ФС, инвариант
 * «file over app» соблюдён полностью.
 */
