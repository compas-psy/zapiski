...
/**
 * Деревья, разрешение на которые есть прямо сейчас, новейшее первым.
 *
 * ── Зачем ───────────────────────────────────────────────────────────────────
 *
 * Выбранная папка записывалась в настройки уже после того, как ответ доходил
 * до JS. Но пока открыт системный выбор, приложение в фоне, и Android вправе
 * убить его процесс — диалог выбора документов тяжёлый, на телефоне со
 * скромной памятью это обычное дело. Тогда результат приходит в НОВЫЙ процесс:
 * разрешение забирается, а тот, кто ждал ответа, уже не существует, и до
 * настроек выбор не доходит. Со стороны — «выбрал папку, а ничего не
 * изменилось».
 *
 ...
 */
export async function persistedSafTrees(): Promise<string[]> {
  const raw = await call<string>(COMMANDS.safPersisted);
 ...
 * восстановление подхватило бы его при следующем запуске и вернуло человека в
 * папку, из которой он ушёл.
 */
export function releaseSafTrees(): Promise<void> {
  return call<void>(COMMANDS.safRelease);
}
