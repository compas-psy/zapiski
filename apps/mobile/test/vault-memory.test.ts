...
describe('открытие хранилища на старте', () => {
  /*
    Свой запас времени, и это не «тест медленный».

    Проверка нарочно ждёт молчащую папку настоящими таймерами — ровно так же,
    как ждёт её запуск на устройстве. Ожидание около трёх секунд, и на общей
    машине под нагрузкой оно упиралось в умолчание vitest (5 с): сторож падал
    через раз и переставал что-либо значить. Ускорять его подделкой времени
    нельзя — тогда он перестанет проверять то, ради чего написан.
  */
  it('папка молчит, а разрешение есть — запуск не забывает выбор', { timeout: 20_000 }, async () => {
