...
describe('nginx: корень — промо, /notes/ — приложение, /auth — по-прежнему приложение', () => {
  ...
  it('HTML отдаётся с кодировкой в заголовке ответа, а не только тегом в разметке', () => {
    /*
     * Счётчик Метрики и вставленный сборкой themeInitScript стоят раньше
     * `<meta charset>` в <head> и вместе отодвигают его дальше байта ~1024, на
     * котором браузер обрывает предпросмотр в поисках кодировки — так и
     * вышло: заголовок вкладки читался как «Р—РоРхРУРхР». Без charset в
     * заголовке ответа это может повториться от любой следующей правки
     * <head>, независимо от того, что написано внутри самой разметки.
     */
    const block = /location ~\* \\\.html\$ \{([^}]*)\}/.exec(vhost)?.[1] ?? '';
    expect(block, 'в vhost нет блока для *.html').not.toBe('');
