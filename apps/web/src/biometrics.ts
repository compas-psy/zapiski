 ...
 *
 * ── ВНИМАНИЕ: сейчас НЕ подключён ────────────────────────────────────────────
 *
 * Код рабочий и остаётся здесь целиком, но `platform.ts` отдаёт
 * `biometrics: null`. Причина не техническая: веб-версия по обещанию продукта
 * — про ЧУЖОЙ компьютер, а платформенный аутентификатор привязан к учётной
 * записи ТОЙ машины. Разблокировку проходит тот, кто проходит на ней Windows
 * Hello или Touch ID, то есть её хозяин. Включив тумблер на чужом ноутбуке,
 * человек оставил бы там ключ от своих зашифрованных заметок.
 *
 * Возврат — одна строка в `platform.ts`. Решение продуктовое, не техническое,
 * поэтому код не удалён.
 *
