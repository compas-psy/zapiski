...
/**
 * Разогрев: первый импорт `@zapiski/app` — это разбор большого пакета, и он
 * не имеет отношения к тому, что здесь проверяется.
 *
 * В preflight наборы идут параллельно по холодному кэшу, и эта плата — три с
 * лишним секунды — доставалась ПЕРВОМУ случаю, который из-за неё вылетал за
 * пятисекундный предел. Тест падал не потому, что платформа посчиталась
 * неверно, а потому, что не успел досчитать. Такой набор врёт о причине, и
 * поднимать ему предел значило бы прятать враньё, а не убирать.
 *
 * Здесь плата вносится один раз и снаружи всех случаев. `vi.resetModules()`
 * внутри прогона сбрасывает реестр модулей, но не кэш разбора Vite, — поэтому
 * разогрев работает.
 */
beforeAll(async () => {
  await import('@zapiski/app');
...
describe('Windows: платформа в запросе ссылки для входа', () => {
  ...
  it('IPC молчит — запуск не встаёт колом, платформа windows', async () => {
    ...
    expect(trace.body['platform']).toBe('windows');
    /* Свой запас времени: случай ЖДЁТ предел ожидания по-настоящему, и общий
       предел набора считался бы от него же. */
  }, 15_000);
});
