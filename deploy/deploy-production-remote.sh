...
# ─────────────────────────────────────────────────────────────────────────────
# Рубильник приёма аналитики (ТЗ §6; маршрут — server/src/routes/analytics.ts).
#
# `ANALYTICS_ENABLED` — ПЕРВЫЙ из четырёх независимых замков приёма, и
# умолчание в коде выключенное (`bool(false)`, server/src/config/env.ts).
# До этой правки его не проставляла ни выкладка, ни compose, ни .env.example —
# то есть на сервере он мог оказаться заданным только рукой человека, а иначе
# `/api/v1/analytics/events` отвечал `analytics_disabled` на всё.
#
# Цена ошибки не в самом флаге: при закрытом первом замке событие не доходит
# даже до `analytics_events` ЗАПИСОК — маршрут отказывает ДО вставки. Значит и
# мост в ПРАКТИКУ переслать ничего не может, как бы правильно он ни был
# настроен. Поэтому рубильник проставляется на КАЖДОЙ выкладке и отдельно от
# resolve_practice_ingest: приём ЗАПИСОК не зависит от соседнего продукта.
#
# Согласия это не ослабляет. Замки независимы: рубильник отвечает на вопрос
# «умеет ли выкладка принимать вообще», `users.analytics_opt_in` — «от этого
# ли человека», и второй проверяется на КАЖДЫЙ запрос. Человек, не давший
# согласия, не отправляет ничего и при включённом рубильнике.
#
# ensure_env, а не upsert_env: выключил руками — значит выключено.
resolve_analytics_gate() {
  ensure_env ANALYTICS_ENABLED 'true'
}
