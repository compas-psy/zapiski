...
describe('поиск', () => {
  ...
  it('фрагмент подсветки берётся из того же места, что и в исходном тексте', () => {
    /*
     * Нормализация индекса обязана СОХРАНЯТЬ ДЛИНУ: по смещениям в
     * нормализованной строке вырезается фрагмент из исходной, и разъехавшись
     * на один знак они покажут человеку обрубок чужого слова.
     *
     * Проверка стоит здесь потому, что нормализацию переписали ради памяти:
     * посимвольная склейка строила дерево из тысяч узлов (81 МБ на корпус в
     * 2,7 МБ), и её заменили нативным `toLowerCase()` с посимвольным запасным
     * путём для букв, у которых строчная форма длиннее прописной. Турецкое «İ»
     * — ровно такая буква, и на ней инвариант ломается первым.
     */
    const index = new InvertedIndex();
    index.rebuild([
      note({
        id: 'turkish',
        path: 'Заметка.md',
        body: '# Заметка\n\nİstanbul и ЁЛКА в одной строке, дальше supervision',
      }),
    ]);
    const fragment = index.search(parseQuery('supervision'))[0]?.fragments[0];
    const range = fragment?.ranges[0];
    /* Подсвеченный кусок обязан быть РОВНО искомым словом: разъедься длины на
       один знак — и подсветка ляжет на соседние буквы. */
    expect(fragment?.text.slice(range?.[0] ?? 0, range?.[1] ?? 0)).toBe('supervision');
    /* И само слово находится, несмотря на соседство «İ». */
    expect(index.search(parseQuery('елка')).map((h) => h.note.id)).toEqual(['turkish']);
  });
