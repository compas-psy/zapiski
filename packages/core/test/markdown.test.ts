...
describe('заголовок (BEHAVIOR §2.2)', () => {
  ...
  /**
   * Прежде заголовком становилась любая первая непустая строка. ITERATION-1 §1
   * это отменил: в список попадал случайный обрывок — заметка, начатая с
   * `###`, так и называлась. Теперь заголовок — только H1, а его отсутствие —
   * законное состояние, и список показывает «Без названия».
   */
  it('строка без решётки заголовком не становится', () => {
    expect(extractTitle('\n\nПросто строка\n\nтело')).toBe('');
    expect(extractTitle('###\n')).toBe('');
    expect(extractTitle('## Раздел\n\nтело')).toBe('');
  });
