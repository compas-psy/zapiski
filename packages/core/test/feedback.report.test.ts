 ...
 *
 * В заметках ЗАПИСОК лежит то, что психолог говорит о клиентах. Утечка одной
...
/** Хранилище, какое бывает у человека: клиенты, диагнозы, личные заметки. */
const NOTES: Record<string, string> = {
  ...
  'Личное/Дневник.md': '# Дневник\n\nсегодня было тяжело после супервизии\n',
  'Супервизия/Случай 14.md': '# Случай 14\n\nконтрперенос, обсудить с супервизором\n',
};
 ...
 * имена собственные и пары соседних слов. Пара «после супервизии» в жалобе на
...
function forbiddenStrings(): string[] {
  ...
  for (const [path, body] of Object.entries(NOTES)) {
    ...
    for (const word of words) {
      /* Имена собственные — сами по себе отличительные: фамилия клиента в теле
...
describe('в теле обращения нет содержимого заметок', () => {
  ...
  it('код ошибки, похожий на путь, в тело не попадает', () => {
    ...
    const report = buildFeedbackReport({
      ...
      diagnostics: {
        ...DIAGNOSTICS,
        errorCodes: ['SYNC_CONFLICT', 'Клиенты/Смирнова А.md', 'read /vault/Личное/Дневник.md'],
      },
    });
