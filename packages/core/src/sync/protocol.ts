/**
 * Протокол ZapiskiCloud (ADR-0003). Типы объявлены здесь один раз и
...
export interface CloudListResponse {
  ...
  cursor?: string;
  /**
   * Пути, удалённые на сервере — приходят только при `includeDeleted=1`.
   *
   * Сервер их отдавал с самого начала, а клиент не спрашивал и не читал:
   * половина протокола удалений была написана и не соединена.
   */
  removed?: string[];
}
