...
export class SyncKeyOnboarding {
  ...
  /**
   * Снять ключ аккаунта на сервере — последний шаг обратного перевода.
   *
   * Зовётся ТОЛЬКО после того, как всё содержимое уже лежит на сервере
   * открытым текстом. Раньше нельзя: сняв ключ, мы потеряем соль аккаунта, а
   * без неё оставшийся шифротекст не расшифровать ничем — локального SMK для
   * этого мало. Именно поэтому порядок в `migrateToPlaintext` такой, а не
   * обратный, и именно поэтому сервер принимает открытый текст по заголовку
   * перевода, а не по факту отсутствия ключа.
   *
   * Материал ключа с УСТРОЙСТВА здесь не стирается намеренно: пока не
   * доказано, что перевод завершился целиком, локальный SMK — единственное,
   * чем можно доделать прерванный переезд.
   */
  async dropAccountKey(): Promise<boolean> {
    const response = await this.fetchImpl(`${this.baseUrl}/api/v1/vault/sync-key`, {
      method: 'DELETE',
    }).catch(() => null);
