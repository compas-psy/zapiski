  ...
  async list(): Promise<RemoteEntry[]> {
    ...
    return entries
      .map((entry) => {
        ...
        /*
         * Осиротевший адрес прошлой схемы. Пока действовало сквозное
         * шифрование, объект адресовался токеном `HMAC(K_manifest, путь)`;
         * шифрование вырезано, но у аккаунта, успевшего его включить, такие
         * объекты на сервере остались, и открыть их больше нечем.
         *
         * Без этой строки клиент принял бы токен за обычный путь и положил бы
         * человеку в папку файлы с именами вроде `a1b2c3…` и нечитаемым
         * содержимым — рядом с настоящими заметками.
         *
         * Заслон работает ТОЛЬКО без ключа: там, где ключ есть, тот же адрес
         * законен и превращается в путь ниже. И он намеренно узкий — ровно
         * 32 знака `[0-9a-f]`; путь настоящей заметки такой формы иметь не
         * может, у неё есть расширение.
         */
        if (this.sync === undefined && SYNC_ADDRESS.test(entry.path)) return null;
