...
export class ZapiskiCloudBackend implements SyncBackend {
  ...
  /**
   * Обратный перевод: шифротекст → открытый текст (решение владельца по MVP).
   *
   * ── Зачем ────────────────────────────────────────────────────────────────
   *
   * Сквозное шифрование уходит из MVP и вернётся позже через внешнюю
   * ключницу. У аккаунта, успевшего пройти онбординг, на сервере лежит
   * шифротекст по токенизированным адресам. Клиент, который больше не
   * шифрует, увидел бы у такого аккаунта ПУСТОЕ облако: по токену он ничего не
   * спрашивает, а сам токен в путь не превращает. Человек потерял бы доступ к
   * своим заметкам из-за смены нашего решения — этого быть не должно.
   *
   * ── Порядок шагов, и почему именно такой ─────────────────────────────────
   *
   *   1. подтянуть манифест — без него токен не превратить в путь;
   *   2. прочитать и расшифровать ВСЁ, ничего не записывая;
   *   3. только теперь писать открытым текстом — по заголовку перевода, ПОКА
   *      ключ аккаунта ещё жив;
   *   4. убрать токенизированные копии и манифест;
   *   5. снять ключ аккаунта — последним.
   *
   * Обратный порядок (снять ключ, потом писать) выглядит проще и ломается на
   * первом же обрыве связи: вместе с ключом уходит соль аккаунта, а без неё
   * оставшийся шифротекст не расшифровать ничем — локального SMK мало.
   * При выбранном порядке обрыв на любом шаге оставляет аккаунт в состоянии,
   * из которого перевод продолжается сам: ключ на месте, соль на месте,
   * переведённое уже открыто, непереведённое ещё читается.
   *
   * ── Отказ ────────────────────────────────────────────────────────────────
   *
   * `null` — перевод не завершён, повторить позже. Половинчатый переезд хуже
   * отказа, поэтому шаг 2 либо собирает ВСЁ, либо не трогает ничего: объект,
   * путь которого восстановить нечем, останавливает перевод целиком, а не
   * выбрасывается по догадке.
   *
   * Число — сколько объектов переведено; ноль означает «переводить было
   * нечего», а не сбой.
   */
  async migrateToPlaintext(dropAccountKey: () => Promise<boolean>): Promise<number | null> {
    if (this.sync === undefined) return 0;
    ...
    // Шаг 2: собрать всё в памяти, не записав ни байта.
    const carried: Array<{ path: VaultPath; address: string; data: Uint8Array }> = [];
    for (const entry of body.entries ?? []) {
      if (entry.path === MANIFEST_ADDRESS) continue;
      if (!SYNC_ADDRESS.test(entry.path)) continue; // уже открытый — трогать нечего
      const path = this.pathOfAddress(entry.path);
      if (path === null) return null; // путь восстановить нечем — отмена целиком
      const found = await this.readAddress(entry.path).catch(() => null);
      if (found === null) return null;
      const data = await this.unseal(path, found.raw);
      if (data === null) return null;
      carried.push({ path, address: entry.path, data });
    }

    // Шаги 3 и 4: записать открытым текстом и убрать токенизированную копию.
    // Оставить её значило бы держать ту же заметку в облаке дважды, причём
    // одну из копий — нечитаемой для клиента без ключа.
    for (const item of carried) {
      await this.putPlain(item.path, item.data);
      await this.deleteAddress(item.address);
    }
    await this.deleteAddress(MANIFEST_ADDRESS);

    // Шаг 5. Не снялся — перевод считается незавершённым и повторится: данные
    // при этом уже в безопасности, открытым текстом по своим путям.
    if (!(await dropAccountKey().catch(() => false))) return null;
    return carried.length;
  }

  /**
   * Записать содержимое по НАСТОЯЩЕМУ пути, без конверта.
   *
   * Только для обратного перевода. Заголовок `X-Migrate-Plaintext` — явное
   * согласие владельца: без него сервер справедливо отбивает открытый текст у
   * зашифрованного аккаунта, чтобы старый клиент не затёр шифротекст молча.
   */
  private async putPlain(path: VaultPath, data: Uint8Array): Promise<void> {
    const response = await this.call(`${VAULT_ENDPOINTS.blob}?path=${encodeURIComponent(path)}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/octet-stream',
        'X-Migrate-Plaintext': '1',
      },
