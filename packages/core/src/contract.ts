...
export interface SyncBackend {
  ...
  /**
   * Опубликовать оглавление хранилища — все пути, которые на той стороне есть.
   *
   * Необязательный порт, и он нужен ровно там, где адрес объекта на сервере
   * НЕ равен пути. Облако Записок после SEC-001 адресует заметки токенами
   * `HMAC(K_manifest, путь)`, чтобы сервер не видел названий папок и
   * заголовков; обратное соответствие держится только у устройств. Значит
   * устройство, которое этого пути ещё не видело (заметка создана на другом),
   * без оглавления не узнает о ней вовсе — список вернёт непрозрачный токен,
   * а не заметку.
   *
   * У папки, WebDAV и Яндекс.Диска порта нет и быть не должно: там адрес и
   * есть путь, публиковать нечего.
   *
   * Отказ здесь не срывает проход: отставшее оглавление ничего не теряет —
   * сами объекты на сервере лежат, — и следующий проход попробует снова.
   */
  pushManifest?(paths: readonly VaultPath[]): Promise<boolean>;
