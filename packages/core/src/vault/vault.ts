...
export class Vault {
  ...
  async read(path: VaultPath): Promise<Note | null> {
    const normalized = normalizePath(path);
    /*
     * Заметка — это `.md` и `.md.enc`, и ничто другое.
     *
     * Проверки здесь не было вовсе: читался любой существующий файл, а байты
     * молча объявлялись markdown-текстом. Пока сюда приходили только заметки,
     * это сходило с рук. Стоило синхронизации начать приносить вложения — и
     * каждая приехавшая картинка проходила через `refresh()` прямиком в
     * индекс: в списке появлялись «заметки» с именами вида `2026-08-14_3591d7`
     * и телом `%PDF-1.4` или `PNG IHDR`, на 124 тысячи «слов». Заказчик
     * увидел это первым же взглядом: «сразу после синхронизации в списке
     * появляются изображения в виде заметок… кажется фейком». Исчезали они
     * при следующей полной пересборке индекса, которая читает только `.md`, —
     * то есть список сам себе противоречил.
     *
     * Отвечать `null` правильнее, чем фильтровать на стороне вызывающих: у
     * вложения нет ни заголовка, ни тела, ни тегов, и любая попытка обойтись
     * с ним как с заметкой — уже ошибка.
     */
    if (!isNotePath(normalized)) return null;
