...
/**
 * Вложение: файл хранилища с известным расширением (SEC-023).
 *
 * ── Почему не «внутри attachments/» ─────────────────────────────────────────
 *
 * Раньше признаком было именно это, и правило пережило собственный повод.
 * Файлы давно ложатся в `Images`, `Audio` и `Other files`, а настройка «рядом
 * с заметкой» кладёт их вообще в любую папку человека. Проверка же осталась
 * про старый каталог — и всё, что снаружи, синхронизация объявляла «не
 * заметкой и не вложением» и не увозила никуда.
 *
 * Заказчик увидел это с двух устройств сразу: «картинки не подгружаются и
 * папок Images, Audio и Other files в принципе нет». Заметки приезжали, а
 * картинки — нет, потому что их не считали вложениями.
 *
 * Защитой остаётся белый список расширений: он и был настоящим предохранителем
 * (`.exe`, `.sh`, `.apk` не пройдут), а каталог только сужал и без того узкую
 * дверь. Служебный `.zapiski/` исключён отдельно.
 */
export function isAttachmentPath(path: VaultPath): boolean {
  const normalized = lower(normalizePath(path));
  /* Служебный каталог — не вложение никогда: там журналы и корзина. */
  if (normalized === lower(META_DIR) || normalized.startsWith(`${lower(META_DIR)}/`)) return false;
