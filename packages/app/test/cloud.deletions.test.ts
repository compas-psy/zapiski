 ...
 * списке.
 *
 * ── Второй заход: папка целиком ─────────────────────────────────────────────
 *
 * Список оказался неполным ровно так, как здесь и предсказано. Про одиночную
 * заметку облаку рассказывали, а про папку — нет: переезд звал `noteMoved`
 * только для ОТКРЫТОЙ заметки, удаление папки не звало `scheduleDelete` вовсе.
 * Заказчик проверял с подключённым облаком и описал обе половины:
 *
 *   «Я перетащил папку ЗАПИСКИ в СИМПАС → папка скопировалась, но не удалилась
 *    из корня»
 *   «удалил папку SignalAI из СИМПАС и она осталась в корне, но потом вернулась
 *    в СИМПАС тоже»
 *
 * Локально обе операции отрабатывали честно. Возвращал файлы синк: он видел на
 * сервере пути, которых здесь больше нет, и — правильно для одиночного файла —
 * скачивал их обратно. Вместе с файлами воскресала и папка.
 */
 ...
 */
async function boot(files: Record<string, string> = FILES) {
...
/**
 * Папка — это не один путь, а все пути внутри неё.
 *
 * Каталогов у синхронизации нет вовсе, она знает файлы. Поэтому проверяем не
 * «позвали метод», а «про КАЖДЫЙ файл в очереди стоит намерение»: ровно этой
 * разницей и отличалась поломка — про один файл (открытый) рассказывали, про
 * остальные молчали.
 */
describe('папка целиком: облако узнаёт про каждый файл внутри', () => {
  const NESTED = {
    'Работа/Смета.md': '# Смета\n\nчисла по смете\n',
    'Работа/Отчёты/Итоги.md': '# Итоги\n\nсводка\n',
    'Работа/Images/схема.png': 'PNG',
  };

  it('переезд папки: старые пути удаляются, новые отправляются', async () => {
    const { app, kindOf } = await boot(NESTED);

    expect(await app.moveFolder('Работа', 'Архив 2025')).toBe('Архив 2025/Работа');

    /* Заметка в самой папке, заметка в подпапке и вложение — три разных пути в
       коде ядра, и каждый из них однажды забывали. */
    expect(await kindOf('Работа/Смета.md'), 'копия осталась бы в облаке').toBe('delete');
    expect(await kindOf('Архив 2025/Работа/Смета.md')).toBe('put');
    expect(await kindOf('Работа/Отчёты/Итоги.md'), 'подпапка не переехала для облака').toBe(
      'delete',
    );
    expect(await kindOf('Архив 2025/Работа/Отчёты/Итоги.md')).toBe('put');
    expect(await kindOf('Работа/Images/схема.png'), 'вложение вернулось бы из облака').toBe(
      'delete',
    );
    expect(await kindOf('Архив 2025/Работа/Images/схема.png')).toBe('put');
  });

  it('переименование папки: то же самое, ни одна заметка не забыта', async () => {
    const { app, kindOf } = await boot(NESTED);

    expect(await app.renameFolder('Работа', 'Проекты')).toBe('Проекты');

    expect(await kindOf('Работа/Смета.md')).toBe('delete');
    expect(await kindOf('Проекты/Смета.md')).toBe('put');
    expect(await kindOf('Работа/Отчёты/Итоги.md')).toBe('delete');
    expect(await kindOf('Проекты/Отчёты/Итоги.md')).toBe('put');
  });

  it('удаление папки с заметками: корзина — это удаление и для облака', async () => {
    const { app, kindOf } = await boot(NESTED);

    expect(await app.deleteFolder('Работа', 'notes-to-trash')).toBe(2);

    expect(await kindOf('Работа/Смета.md')).toBe('delete');
    expect(await kindOf('Работа/Отчёты/Итоги.md')).toBe('delete');
    /* Вложение уезжает вместе с папкой и в корзину не попадает — значит его
       удаление обязано быть заказано отдельно, иначе оно приедет обратно и
       воскресит папку. */
    expect(await kindOf('Работа/Images/схема.png'), 'папка вернулась бы вложением').toBe('delete');
  });

  it('удаление только папки: заметки поднялись к родителю — облако знает оба пути', async () => {
    const { app, kindOf } = await boot(NESTED);

    await app.deleteFolder('Работа', 'notes-to-parent');

    expect(await kindOf('Работа/Смета.md')).toBe('delete');
    expect(await kindOf('Смета.md')).toBe('put');
    expect(await kindOf('Работа/Images/схема.png')).toBe('delete');
  });
});
