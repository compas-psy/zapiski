...
describe('папки', () => {
  ...
  it('перенос — это ПЕРЕНОС: на старом месте не остаётся ничего', async () => {
    /*
     * Заказчик: «Если перетащил в меню папку в папку, то она копируется, а не
     * перемещается». Так и было, и причина не в перетаскивании, а в самом
     * переносе: переезжали только ЗАМЕТКИ из индекса. Вложения, файлы чужих
     * форматов и пустые подпапки оставались на месте, каталог-источник от
     * этого не пустел, и удалить его не удавалось — а неудача проглатывалась
     * молча. На экране: папка есть и там, и там.
     */
    const app = await boot({
      'Практика/Заметка.md': '# Заметка\n\nтекст\n',
      'Практика/Images/снимок.png': 'PNG',
      'Практика/2026/Старое.md': '# Старое\n',
      'Практика/договор.pdf': 'PDF',
    });
    await app.createFolder('', 'Работа');

    expect(await app.moveFolder('Практика', 'Работа')).toBe('Работа/Практика');

    const storage = app.vaultRef!.storage;
    /* Ничего не осталось на старом месте — ни файла, ни каталога. */
    expect(await storage.stat('Практика'), 'папка осталась на старом месте').toBeNull();
    expect(await storage.stat('Практика/договор.pdf')).toBeNull();
    expect(await storage.stat('Практика/Images/снимок.png')).toBeNull();
    /* И всё приехало на новое, включая то, что не является заметкой. */
    expect(await storage.stat('Работа/Практика/договор.pdf')).not.toBeNull();
    expect(await storage.stat('Работа/Практика/Images/снимок.png')).not.toBeNull();
    expect(app.getState().notes.map((note) => note.path).sort()).toEqual([
      'Работа/Практика/2026/Старое.md',
      'Работа/Практика/Заметка.md',
    ]);
  });
