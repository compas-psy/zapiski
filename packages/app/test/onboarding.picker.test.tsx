...
describe('шаг 2: выбор места для заметок', () => {
  ...
  it('Windows: вариант «Облако Записок» снова предлагается', () => {
    /*
      Проверка возвращена вместе с самим вариантом. Она пережила два
      разворота продукта: сначала выбор облака вёл к окну выбора папки, хотя
      кнопка обещала «Дальше» (дефект, исправлен); потом вариант убрали
      вовсе — сперва выключателем SEC-001, затем платформенным замком. Оба
      замка сняты, и вариант обязан быть на месте.
    */
    const base = createTestHost();
    const pick = vi.fn(async () => null);
    const host = {
