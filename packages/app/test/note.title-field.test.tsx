...
async function mountNote(files: Record<string, string>, path: string) {
  ...
  const title = await screen.findByRole<HTMLInputElement>('textbox', { name: 'Название заметки' });
...
describe('поле заголовка', () => {
  ...
  it('у заметки без H1 поле пустое, а текст цел', async () => {
    ...
    const { title } = await mountNote({ 'Чужая.md': 'просто текст\nвторая строка\n' }, 'Чужая.md');
    expect(title.value).toBe('');
    expect(document.querySelector('.cm-content')?.textContent).toContain('просто текст');
  });
  ...
  it('Enter в названии не ломает строку', async () => {
    const { title } = await mountNote({ 'Заметка.md': '# Название\n\nтекст\n' }, 'Заметка.md');
