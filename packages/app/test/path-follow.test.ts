 ...
 * 5. переезд ПАПКИ — переименование, перемещение, удаление «только папку».
 *
 * Пятый пункт добавлен по отказу заказчика и стоит особняком: во всех
 * предыдущих путь менялся у одной заметки, а тут — у всех внутри разом.
 * Ровно на этом «разом» и вышел промах: код звал следование за путём для
 * ОТКРЫТОЙ заметки, а про остальные молчал. Видно это было и без облака —
 * «недавние» держали строки, которые никуда не открываются, — но заметили
 * с облаком, где молчание превращается в возвращённую из синка папку
 * (`cloud.deletions.test.ts`).
 */
...
describe('состояние следует за путём заметки', () => {
  ...
  it('переезд папки: за путём следует КАЖДАЯ заметка, а не только открытая', async () => {
    const app = await boot();
    const first = await noteTitled(app, 'Первая');
    await app.move(first, 'Практика');
    const second = await noteTitled(app, 'Вторая');
    await app.move(second, 'Практика');
    /* Открыта вторая, первая — только в «недавних». Именно эта половина и
       оставалась с покойником: строка в списке есть, открывать нечего. */
    expect(app.getState().lastOpened).toContain('Практика/Первая.md');

    await app.moveFolder('Практика', 'Архив');

    expect(openPath(app)).toBe('Архив/Практика/Вторая.md');
    expect(app.getState().lastOpened, 'в «недавних» остался мёртвый путь').not.toContain(
      'Практика/Первая.md',
    );
    expect(app.getState().lastOpened).toContain('Архив/Практика/Первая.md');
  });

  it('удаление «только папку»: маршрут не бросает заметку, а едет с ней', async () => {
    const app = await boot();
    const path = await noteTitled(app, 'Останется');
    await app.move(path, 'Практика');

    await app.deleteFolder('Практика', 'notes-to-parent');

    /* Заметка поднялась к родителю — уводить с неё экран не за что. Раньше
       проверка маршрута стояла до следования за путём и отправляла в список. */
    expect(openPath(app)).toBe('Останется.md');
  });
