...
export class AppController {
  ...
  /**
   * @param options.byHand человек нажал «Синхронизировать сейчас».
   *
   *   Различие обязательное, и куплено оно дорого. `syncNow` зовётся ещё и
   *   САМ — при возврате в приложение, по таймеру, после правок, — и объяснять
   *   отсутствие бэкенда на каждом таком заходе значит показывать тост, о
   *   котором никто не просил. На телефоне он ложится поверх панели
   *   начертаний и делает «Ж», «К», «Ч», «З» ненажимаемыми: браузерная
   *   проверка `check-inline-format.mjs` поймала это через полчаса после того,
   *   как я объяснение добавил.
   *
   *   Поэтому вслух — только по нажатию. Автоматический заход по-прежнему
   *   выходит молча: он и не обещал ничего человеку.
   */
  async syncNow(options: { announce?: boolean; byHand?: boolean } = {}): Promise<void> {
    const engine = this.engine;
    if (!engine) {
      /*
       * Молчаливый отказ ПО НАЖАТИЮ — тот же дефект, за который мы уже
       * платили: человек нажимает «Синхронизировать сейчас» и не получает
       * НИЧЕГО, даже причины, и заключает, что сломано приложение.
       *
