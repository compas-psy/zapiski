...
export class AppController {
  ...
  async saveQuickNote(text: string, folder?: string): Promise<VaultPath | null> {
    ...
    /*
     * Первая строка становится заголовком — ровно то, что обещает онбординг:
     * «Первая строка станет заголовком». Документ собирается тем же
     * `joinTitle`, которым его собирает редактор, поэтому файл выходит
     * неотличимым от обычной заметки: `# Позвонить в понедельник`.
     *
     * Без этого записка получала имя «Без названия»: заголовок в markdown — это
     * `#`-строка, и голый текст ею не считается. Человек увидел бы в списке
     * столбец безымянных записок и решил, что они не сохранились.
     */
    const [first = '', ...rest] = body.split('\n');
    const document = joinTitle(first.trim(), rest.join('\n').trim());
    /* `create` возвращает саму заметку — путь берём из неё: у остальных вызовов
       этого метода то же соглашение. */
    const note = await vault.create({ body: document, ...(folder ? { folder } : {}) });
    await vault.rebuild();
    await this.refresh();
