...
function VaultLocationChoice(): ReactNode {
  ...
  return (
    <>
      ...
      {/*
        Настоящий адрес места — под именем и мелким шрифтом.

        Имя — это вывод приложения о месте, адрес — само место. Три круга
        подряд заказчик видел «Записки» там, где только что указал свою папку,
        и отличить «выбор не применился» от «имя совпало» было нечем. Теперь
...
function PlusSection(): ReactNode {
  ...
  return (
    <>
      ...
      <Button onClick={() => app.navigate({ name: 'paywall' })}>
        {strings.paywall.trial(trialDaysFor(Date.now()))}
      </Button>
    </>
  );
}
