 ...
 *
 * Р1 и Р2 мастер-ТЗ: продукт называется ЗАПИСКИ — самостоятельный бренд, без
 * приставки; издатель СИМПАС существует только в сторе, счетах и юр. текстах;
 * слов «КОМПАС» и «Окоём» в интерфейсе нет вовсе.
 *
 ...
 *
 * Исключений больше нет. Раньше из-под шаблона выводился `KompasCloud…` —
 * имя класса синка; мастер-ТЗ переименовало и облако («Облако Записок», Р2),
 * поэтому класс теперь `ZapiskiCloudBackend`, а шаблон ловит `kompas` в любом
 * окружении.
 *
 * `cmpas` (домен `cmpas.ru`, `identifier` `ru.cmpas.zapiski`) под шаблон не
 * подходит и ловиться не должен: это адреса, а не имя в интерфейсе. Публичный
 * домен продукта — открытый флаг мастер-ТЗ, и решается он не здесь.
 */
const CYRILLIC = /КОМПАС/gi;
const LATIN = /kompas/gi;
...
describe('отменённый бренд не вернулся в оболочки (Р1, Р2)', () => {
  ...
  it('ни одной «КОМПАС» в apps/**', () => {
    expect(
      ...
      'Отменённый бренд снова в оболочке. Продукт называется ЗАПИСКИ, ' +
        'издатель СИМПАС — только в сторе, счетах и юр. текстах (Р1, Р2).',
    ).toEqual([]);
  });
