...
describe('перф-бюджеты (ARCHITECTURE §4)', () => {
  ...
  /**
   * Бюджет открытия — по МЕДИАНЕ, а не по худшему прогону.
   *
   * Раньше проверялся худший из пяти, «включая самый первый, холодный». На
   * выделенной машине это работало, а на общей (CI, контейнер разработки под
   * нагрузкой) первый прогон меряет не наш код, а прогрев JIT и чужие
   * процессы: на НЕИЗМЕНЁННОМ дереве худший прогон гулял 123 → 232 мс при
   * медиане 64 → 116. Сторож падал через раз и переставал что-либо значить —
   * ровно то, ради чего его не стоило заводить.
   *
   * Поэтому два утверждения вместо одного:
   *   · медиана держит прежний бюджет 150 мс — это и есть «сколько стоит
   *     открытие», и число устойчиво от прогона к прогону;
   *   · худший прогон не должен превышать 500 мс — этот потолок ловит
   *     настоящий провал (разбор всего документа вместо экрана), но не шум.
   */
  it('открытие заметки 1 МБ — меньше 150 мс по медиане', () => {
    const runs: number[] = [];
    for (let i = 0; i < 5; i++) {
    ...
    console.log(
      `открытие заметки 1 МБ: медиана ${median.toFixed(1)} мс · худший ${worst.toFixed(1)} мс` +
        ` (прогоны: ${runs.map((r) => r.toFixed(1)).join(', ')})`,
    );
    expect(median, 'открытие заметки подорожало').toBeLessThan(150);
    expect(worst, 'разовый прогон вышел за всякие рамки — это уже не шум').toBeLessThan(500);
  });
