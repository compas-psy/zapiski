...
describe('вставка в редактор', () => {
  ...
  describe('вставка не переопределяет соседний блок (MVP P0 §9)', () => {
    ...
    /*
     * P1-аудит closure-pass: «paste с ограждённым кодом проглатывается в
     * некоторых позициях». Защита выше закрывает опасность СВЕРХУ (первая
     * строка вставки переопределяет блок над курсором) — здесь то же самое,
     * только СНИЗУ. Вставка многострочная и заканчивается закрывающей
     * оградой ``` ``` ```; если курсор стоял не в конце исходной строки, её
     * хвост приклеивался к этой закрывающей ограде — по CommonMark строка
     * закрытия обязана состоять ТОЛЬКО из ограды, лишний символ на ней и
     * блок кода не закрывается вовсе, поглощая всё, что было ниже, до конца
     * документа: вставленный код остаётся, а весь текст НИЖЕ бесследно
     * исчезает из видимого документа, становясь содержимым код-блока.
     */
    it('вставка ограждённого кода перед хвостом строки не проглатывает всё, что ниже', () => {
      const v = makeView('Line one\nMore text here', { selection: { anchor: 9 } });
      view = v;
      paste(v, clipboard({ text: '```js\ncode\n```' }));

      expect(v.state.doc.toString()).toBe('Line one\n```js\ncode\n```\nMore text here');
    });

    it('вставка ограждённого кода в конце документа не получает лишней строки', () => {
      const v = makeView('Paragraph\n\n', { selection: { anchor: 11 } });
      view = v;
      paste(v, clipboard({ text: '```js\ncode\n```' }));
    ...
    it('многострочная вставка БЕЗ ограды на конце не получает лишней строки после себя', () => {
      const v = makeView('Line one\nMore text here', { selection: { anchor: 9 } });
      view = v;
      paste(v, clipboard({ text: 'первая\nвторая' }));

      // Обычный многострочный текст (без закрывающей ограды на конце) не
      // рискует «проглотить» ничего — защита узкая, не трогает этот случай.
      expect(v.state.doc.toString()).toBe('Line one\nпервая\nвтораяMore text here');
    });
  });
