...
const styles = new StyleModule({
  '.zp-panel': {
    ...
    /*
     * Кнопок больше, чем ширины экрана, — переносим, а не прокручиваем.
     *
     * Прокрутка вбок здесь была и не сработала. Заказчик: «в редакторе записки
     * меню не помещается по ширине в экран». На 390 панель просит 434 px, и
     * последняя кнопка (формула) оставалась за кромкой; на 360 и 320 за
     * кромкой оставалось больше. Хуже, что добраться до них было почти нечем:
     * у кнопок стоит `touch-action: none` — иначе они не нажимаются вовсе
     * (см. ниже), — и тащить панель приходилось за узкие промежутки между
     * пилюлями. Кнопка, до которой нельзя дотянуться, ничем не лучше
     * отсутствующей.
     *
     * Перенос показывает все кнопки при любой ширине устройства и ничего не
     * прячет за жестом. Плата — вторая строка на узком экране: на 390 панель
     * занимает две строки вместо одной. Это честный обмен, и он обратим —
     * на широком экране всё по-прежнему в одну строку.
     */
    maxWidth: '100%',
    flexWrap: 'wrap',
  ...
  '.zp-panel__btn': {
    ...
    /*
     * Касание по кнопке — только касание, и ничего больше.
     *
     * Заказчик: «появление виджета редактирования таблицы нестабильно».
     * Воспроизведено в настоящем Chromium с тач-эмуляцией: касание, при
     * котором палец сместился на 16 px и больше, приходит как `pointerdown` +
     * `pointercancel` — без `pointerup` и без `click`. Кнопка не срабатывает
     * вовсе. 16 px на телефоне с плотностью 3 — это меньше полутора
     * миллиметров пути пальца; обычное нажатие большим пальцем с другого края
     * экрана столько проходит запросто. Отсюда и «через раз».
     *
     * Отменяет жест не наша панель, а браузер: он видит, что палец начал
     * движение внутри прокручиваемого предка, и отдаёт жест прокрутке. Что
     * характерно, прокручивать нечего — на 390 px панель помещается целиком
     * (`scrollWidth === clientWidth`), но `overflow-x: auto` делает её
     * скролл-контейнером независимо от содержимого, и цена платится всегда.
     *
     * `touch-action: none` говорит браузеру: жесты, начавшиеся здесь, —
     * не прокрутка, решать нечего, отменять нечего. Плата: когда кнопок
     * действительно больше ширины экрана, тащить панель вбок нужно за
     * промежутки между пилюлями, а не за сами кнопки. Обмен честный:
     * ненажимающаяся кнопка хуже, чем менее удобная прокрутка.
     *
     * Различить «касание со сносом» и «намеренную протяжку» по событиям
     * нельзя: отмена приходит ровно на пороге в 16 px и с обнулёнными
     * координатами — сколько бы палец ни прошёл дальше, кнопка видит те же
     * 16 px (проверено).
     */
    touchAction: 'none',
  ...
  '.zp-panel__item': {
    ...
    textAlign: 'start',
    cursor: 'pointer',
    /* Та же болезнь, что у кнопок панели, но лечение мягче: меню бывает
       длиннее экрана (заголовки H1–H6, наборы эмодзи), и вертикальную
       прокрутку внутри него отнимать нельзя. `pan-y` оставляет прокрутку и
       забирает у браузера повод отменить касание из-за бокового сноса
       пальца — а он тут и мешал. Снос вдоль прокрутки по-прежнему считается
       прокруткой, и это правильно: человек листает, а не выбирает. */
    touchAction: 'pan-y',
  },
        ...
        */}
        {styling ? (
          ...
          <>
            ...
            */}
            {/*
              Подпись меняется вместе с назначением.
              Заказчик: «проверь мастер таблиц — у меня его нигде нет, он нигде
              не появляется». Мастер был и работал: браузерный прогон открывает
              его на телефоне и на десктопе. Не находился он потому, что одна и
              та же кнопка делает два разных дела — снаружи таблицы вставляет
              её, внутри открывает редактор, — а называлась всегда одинаково.
              Кнопка, которая не говорит, что сделает, ничем не отличается от
              отсутствующей.
            */}
            <MenuButton
              label={table ? copy.tableEdit : copy.table}
              expanded={open === 'table'}
 ...
 * «Aa» — стиль абзаца.
 *
 * Строчная буква рисуется чашей и штамбом, а не одной дугой: прежний значок
 * читался как «Ar», и заказчик прочитал его именно так. Пропорция обычная для
 * шрифта: высота строчной — половина прописной.
 */
const IconStyle = (): ReactElement => (
  <svg {...ICON}>
    <path d="M3 19 8 5l5 14" />
    <path d="M4.8 15h6.4" />
    <circle cx="18.3" cy="16.2" r="2.7" />
    <path d="M21 13.5V19" />
  </svg>
);
