...
/** Голая строка-ограда код-блока: только маркер, ничего больше — то, чем
 * ЗАКРЫВАЕТСЯ ``` ```/`~~~`-блок по CommonMark (лишний символ на строке —
 * и строка перестаёт быть закрывающей). */
const BARE_FENCE_LINE = /^[\t ]{0,3}(`{3,}|~{3,})[\t ]*$/;

/**
 * Нужна ли пустая строка ПОСЛЕ вставленного текста `pastedText` (P1-аудит:
 * «paste с ограждённым кодом проглатывается в некоторых позициях»).
 *
 * Симметрично `pasteNeedsBlankLineBefore`, только с другой стороны:
 * защита ДО этой правки закрывала опасность «первая строка вставки
 * переопределяет блок НАД точкой курсора», но ничего не знала про хвост
 * ИСХОДНОЙ строки, оставшийся ПОСЛЕ точки вставки. Если вставка
 * многострочная и заканчивается закрывающей оградой код-блока, а курсор
 * стоял не в конце строки — хвост исходной строки приклеивается к самой
 * этой закрывающей ограде: `` ``` `` + «остаток строки» уже не «голая»
 * закрывающая строка по правилам CommonMark, блок кода не закрывается и
 * поглощает всё, что было ниже, до конца документа или следующей
 * настоящей ограды — вставленный код остаётся на месте, а вот весь текст
 * НИЖЕ бесследно исчезает из видимого документа, становясь содержимым
 * код-блока.
 */
export function pasteNeedsBlankLineAfter(pastedText: string): boolean {
  if (!pastedText.includes('\n')) return false;
  const lines = pastedText.split('\n');
  const lastLine = lines[lines.length - 1] ?? '';
  return BARE_FENCE_LINE.test(lastLine);
}
