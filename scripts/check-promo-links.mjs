...
/*
 * «Открыть в браузере» обязана вести в приложение, а не в саму промостраницу.
 * До переноса приложения на /notes/ (корень отдан промо) кнопка правилась
 * невнимательно и осталась указывать на «/» — то есть вела саму на себя.
 * Регекс без глобального флага находит только первое несовпадение; страница
 * повторяет кнопку дважды (наверху и в подвале), поэтому проверяем счётчик,
 * а не просто «нашлось хоть одно совпадение».
 */
const openInBrowser = [...promo.matchAll(/href="([^"]*)">Открыть в браузере/g)].map(
  (match) => match[1],
);
if (openInBrowser.length === 0) {
  problems.push('на промостранице нет кнопки «Открыть в браузере»');
} else if (openInBrowser.some((href) => href !== '/notes/')) {
  problems.push(
    `кнопка «Открыть в браузере» ведёт на «${openInBrowser.find((href) => href !== '/notes/')}», ` +
      'а не на /notes/ — с переноса приложения корень отдан промо, и кнопка вела бы сама на себя',
  );
}
