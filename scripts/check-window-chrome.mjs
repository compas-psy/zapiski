#!/usr/bin/env node
/**
 * Сторож окна оболочки: две вещи, которые ломаются из конфига, а видны только
 * на устройстве, — системная рамка поверх своей строки заголовка и
 * перехваченное перетаскивание.
 *
...
for (const shell of SHELLS) {
  ...
  /*
    Перетаскивание внутри страницы против перетаскивания файлов В окно.
    `dragDropEnabled: true` отдаёт перетаскивание оболочке — она ждёт файлы из
    проводника и ГЛУШИТ обычный HTML-перенос внутри страницы. Заказчик на
    Windows: «не работает перетаскивание заметки в папку». Строка списка честно
    объявляла себя переносимой, папка честно ждала `drop`, а до неё ничего не
    доходило: события забирало окно. На Android это уже выключено — там дефекта
    и не было.

    Приём файлов из проводника у нас не заявлен нигде: импорт идёт диалогом
    выбора, вложения — тем же диалогом. Значит терять нечего.
  */
  for (const window of windows) {
    if (window.dragDropEnabled === true) {
      problems.push(
        `${shell.name}: окно «${window.label}» перехватывает перетаскивание ` +
          `(dragDropEnabled: true в ${shell.conf}) — перенос заметки в папку ` +
          'внутри страницы работать не будет: события забирает оболочка',
      );
    }
  }
...
if (problems.length > 0) {
  console.error('Окно: настройки оболочки расходятся с обещанным поведением:');
  for (const problem of problems) console.error(`  · ${problem}`);
  process.exit(1);
}

console.log(
  'Хром окна: системная рамка не вернётся поверх своей строки заголовка, ' +
    'перетаскивание внутри страницы не перехвачено',
);
