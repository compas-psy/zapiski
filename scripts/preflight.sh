...
# Настоящей клавиатурой: список → Enter × 2 → абзац не возвращается в список,
# Setext снимается «Обычным текстом» (MVP P0 §13). Модульные тесты зовут
# `listNewline`/`setHeading` напрямую — этот прогон доказывает дорогу от
# нажатия клавиши до них.
step 'Живой браузер: редактор, список → абзац, Setext' node scripts/editor-regression.mjs --strict
# FolderPicker настоящей клавиатурой и мышью: move note/folder, вложенная
# папка, root, одинаковые basename в разных ветках, expand/collapse, шеврон
# не выбирает узел (только раскрывает), Escape, keyboard navigation. Заведён
# после structural edge-case, где текущая папка при переносе пропадала из
# дерева, а её дети поднимались на чужой уровень — модульные тесты в
# happy-dom доказывают DOM-структуру, но не настоящий правый клик и не то,
# что клик по шеврону и клик по названию строки — разные зоны.
step 'Живой браузер: FolderPicker' node scripts/folder-picker-regression.mjs --strict
...
# Крупность знака в готовых файлах — приёмка ICON-SPEC §6, измерением по
# пикселям. Проверка выше сверяет растр с ИСХОДНИКОМ и молчит, если ошибка
# уже в исходнике; эта меряет то же, что померил бы человек линейкой.
# Знак один на девять сервисов СИМПАСа, и разъехаться в нём может ровно одно —
# крупность: рядом на экране разница читается как иконки разных компаний.
step 'Крупность знака в иконках' node scripts/check-icon-scale.mjs --strict
