 ...
 * rgb(, rgba(, hsl(, hsla(. Исключения — TOKEN_FILES (сгенерированные из
 * design/tokens.json файлы и производный слой) и TOKEN_DIRECTORIES (знак
 * сервиса: файл дизайна, а не код). Обоснование каждого исключения — рядом
 * с ним.
 *
...
/** Единственные места в репозитории, где цветовые литералы разрешены. */
const TOKEN_FILES = [
  // СГЕНЕРИРОВАН из design/tokens.json — единственного артефакта передачи
  // «дизайн → код» (tz/ZAPISKI_TZ_3_Agents.md §6). Руками не правится:
  // `build-tokens.mjs --check` в преflight падает на любой ручной правке.
  'packages/ui/src/styles/tokens.generated.css',

  // Производный слой: hover, кольца, подсветка. Литералов здесь ровно три —
  // чёрный и белый как направление подмешивания в color-mix() и явный hover
  // граната из эталона. Их сторожит packages/ui/test/styles.test.ts.
...
const TOKEN_DIRECTORIES = [
  // Знак сервиса `zapiski.svg` — файл дизайна, а не код: гранатовая плашка и
  // дерево внутри него заданы литералами, потому что svg не читает CSS-токены
  // приложения. Контур дерева трогать запрещено (DS-ALIGNMENT §9 в части знака
  // остался в силе), поэтому исключение — на каталог, а не на строку.
