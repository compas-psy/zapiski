 ...
 *
 *  1. **Дорога.** До формы человек добирается кнопкой в библиотеке (и второй —
 *     в «Настройках → О приложении»). Кнопку можно убрать, переименовать или
 *     спрятать под условие — и все модульные тесты останутся зелёными, потому
 *     что они не нажимают её, а вызывают метод. Ровно так однажды пропала
 *     Справка: экран был готов, покрыт тестом и никуда не подключён.
...
for (const viewport of VIEWPORTS) {
  for (const theme of ['light', 'dark']) {
    ...
    /*
     * Основной путь — библиотека: на альфа-тестировании вход должен быть в
     * ОДНО касание, а не в три сквозь настройки. Раскрываем её, если она не
     * постоянная панель (телефон), и жмём.
     */
    ...
    check(
      (await open.count()) > 0,
      `${where}: в библиотеке нет входа в обратную связь — на альфе так не напишет никто`,
    );
